...
export async function getAiDecisions(): Promise<AiDecision[]> {
  ...
  const response = await client.chat.completions.create({
    ...
    messages: [
      {
        role: "user",
        content: `あなたは仮想資金で運用するスウィングトレーダーです（数日〜数週間の保有を想定）。以下の情報をもとに、各銘柄に対する売買判断を行ってください。
...
- 当日すでに前日終値から+3%超の上ギャップで寄った銘柄は「ギャップ後の失速」を掴みやすい。順張りでも新規BUYは見送るか押し目を待つ。
- 直近1ヶ月(20日)で+80%超など過度に高騰した銘柄は“パラボリック”で反落・急落しやすい。RSIが70未満でも新規BUYは見送る（高騰し切りを掴まない）。
- 地合いが悪く良質な買い場が無い日は、現金を遊ばせる方を選んでよい（無理な順張りで負けない）。

# 指示
- スウィング目的で数日〜数週間の値動きを狙ってください（当日中の手仕舞いは前提としません）。
